...
1. A window whose title contains `Infinity POS` is looked for; if found it is restored (when minimised) and brought to the front. If none is found, a line `No window found with title matching: Infinity POS` goes to the log — and the clicks are still made, at the same screen positions, on whatever is there.
